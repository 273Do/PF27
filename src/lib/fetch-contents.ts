...
export type ProfileOnlyObj = Omit<ProfileObj, "skills" | "links">;
...
export const getProfileLinks = async (queries?: MicroCMSQueries) => {
  const res = await client.getObject<Pick<ProfileResponse, "links">>({
    endpoint: "profile",
    queries,
  });
